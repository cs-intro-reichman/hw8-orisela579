...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    public boolean addUser(String name) {
        if(this.getUser(name) != null || this.userCount == this.users.length) return false;
    ...
    public String recommendWhoToFollow(String name) {
        ...
        for(int i = 0; i < userCount; i++){
            if(this.users[i].getName() != name){
                if(getUser(name).countMutual(this.users[i]) > big){
                    name2 = this.users[i].getName();
                    big = getUser(name).countMutual(this.users[i]);
                }
            }
        }
        return name2;
    ...
    public String mostPopularUser() {
        ...
        int count = 0;
        for(int i = 0; i < userCount; i++){
            if (followeeCount(this.users[i].getName()) > count) {
                name = this.users[i].getName();
                count = followeeCount(this.users[i].getName());
            }
        }
        return name;
    }
